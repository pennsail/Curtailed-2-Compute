...
The `eval/` directory is **not** part of the public reproducibility workflow at this stage. It contains exploratory battery and workload evaluation code that depends on non-public Azure VM traces and should be reviewed separately before any public release.
...
└── eval/                              # Experimental, not in public workflow
```
...
- battery and workload evaluation artifacts derived from non-public Azure traces
...
- Treat `eval/` as experimental until the battery/workload path is reworked to use public or synthetic data.
...
For questions about the public workflow, open an issue in the repository. For battery and workload evaluation components, coordinate with the maintainers and Jiali Xing before publishing or reusing `eval/` materials.
